...
# TELEGRAM_GROUPCHAT_IDS = [-1002690273043]
...
#         if msg.topic == 'factory/gearbox1/input/rpm':
#             # Fetch the last two RPM entries
#             recent_rpms = gear_value.objects.filter(topic=msg.topic).order_by('-timestamp')[:2]

#             if recent_rpms.count() == 2:
#                 latest = recent_rpms[0]
#                 previous = recent_rpms[1]

#                 rpm_diff = abs(latest.payload - previous.payload)

#                 # Avoid alert if both values are zero
#                 if previous.payload == 0 and latest.payload == 0:
#                     return

#                 # Trigger Telegram alert if RPM increases by more than 20
#                 if rpm_diff > 20:
#                     message = (
